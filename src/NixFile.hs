...
hacknixRegion =
  ...
  where 
        ...
        -- input either REGION HACK_NIX code or dist/ *.nix code generated by hack-nix --to-nix
        splitC :: [ BS.ByteString ] -> ([BS.ByteString], [BS.ByteString], [BS.ByteString])
        splitC list = case list of
          [] -> ([],[],[]) -- initial population of REGION HACK_NIX
          _ -> case break (((BS.pack "srcFile") `BS.isPrefixOf`) . dropSpaces) list of
        ...
        action' i@(IRegionData ind opts contents map' _)
               path workAction args
               repoDir
               = do
